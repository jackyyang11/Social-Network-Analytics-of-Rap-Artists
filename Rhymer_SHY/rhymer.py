...
def read_lyrics(lyrics_dir='lyrics_nan', artist=None, album=None, print_stats=False, language='en-us', lookback=15):
    '''
    ...
    for a in artists:
        ...
        # remove nan
        rls = np.array(rls)
        rls = rls[np.logical_not(np.isnan(rls))]
        mean_rl = np.mean(rls)
    ...
    for i in range(len(artist_scores)):
        ...
        new = pd.DataFrame({'score': [artist_scores[i]], 'name': [name]})
        df = df.append(new)
